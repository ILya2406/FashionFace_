...
public sealed class HandleUserToUserInvitationCreatedOutboxConsumer(
    ...
    ILogger<HandleUserToUserInvitationCreatedOutboxConsumer> logger
) : IConsumer<HandleUserToUserInvitationCreatedOutbox>
{
    private const int BatchSize = 5;

    public async Task Consume(
        ConsumeContext<HandleUserToUserInvitationCreatedOutbox> context
    )
    {
        var eventMessage =
            context.Message;
        ...
        var outboxConsumerName =
            nameof(HandleUserToUserInvitationCreatedOutboxConsumer);
        ...
        while (outboxList.IsNotEmpty())
        {
            ...
            {
                var command =
                    new HandleUserToUserInvitationCreatedNotificationOutbox(
